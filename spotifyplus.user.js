...
/* Reduce oversized hero/header voids on Spotify home while retaining artwork pages. */
...
/* Keep Premium prompts visually secondary without removing Spotify functionality. */
...
/* Player: preserve large controls, add clear separation from bottom navigation. */
...
`;
function install(){document.documentElement.classList.add(ROOT);let s=document.getElementById(STYLE);if(!s){s=document.createElement('style');s.id=STYLE;document.documentElement.appendChild(s)}s.textContent=css;document.getElementById('spotifyplus-toggle')?.remove()}
install();
