...
// @version      1.2.0
// @description  V0.8 mobile skin with V0.9 native-style Library and V1.1 status nav.
...
(() => {
'use strict';
const VERSION='1.2',ROOT='spotifyplus-mobile',STYLE='spotifyplus-style';
...
/* V1.1 bottom-right SP+ status/version treatment */
...
/* V0.9 native-style Library */
#${PANEL}{position:fixed;left:0;right:0;top:0;bottom:150px;z-index:2147483500;background:#101010;color:#fff;font-family:system-ui,-apple-system,Segoe UI,sans-serif;display:none;overflow:auto}
...
#${PANEL} .spl-iconbtn{width:46px;height:46px;border:0;background:transparent;color:#fff;font-size:30px;line-height:1;border-radius:50%;display:grid;place-items:center}
...
#${PANEL} .spl-more{border:0;background:transparent;color:#aaa;font-size:26px;width:40px;height:44px}
...
function ensureLibrary(){if(document.getElementById(PANEL)||!document.body)return;const el=document.createElement('div');el.id=PANEL;el.innerHTML=`<div class="spl-wrap"><div class="spl-head"><div class="spl-avatar">SP</div><h1>Your Library</h1><button class="spl-iconbtn" id="spl-search-btn" aria-label="Search">⌕</button><button class="spl-iconbtn" id="spl-add-btn" aria-label="Add">＋</button><button class="spl-manage" id="spl-manage">Manage</button></div><div class="spl-search" id="spl-search"><input id="spl-search-input" placeholder="Search your SP+ Library"></div><div class="spl-chips" id="spl-chips"></div><div class="spl-sort"><span>↕</span><span>Recents</span></div><div class="spl-list" id="spl-list"></div><div class="spl-note">SP+ Library is saved locally in this browser. It does not sync with Spotify's account library.</div></div>`;document.body.appendChild(el);
...
el.querySelector('#spl-manage').onclick=()=>{if(!readLib().length)return;if(confirm('Manage SP+ Library: remove all saved items?')){writeLib([]);renderLibrary()}};renderLibrary()}
...
function install(){document.documentElement.classList.add(ROOT);document.getElementById('spotifyplus-status')?.remove();let s=document.getElementById(STYLE);if(!s){s=document.createElement('style');s.id=STYLE;document.documentElement.appendChild(s)}s.textContent=css;ensureLibrary();dismissLibraryPrompt();wireLibraryNav();replaceGetAppWithStatus()}
install();
new MutationObserver(install).observe(document.documentElement,{childList:true,subtree:true});
[400,900,1600,3000].forEach(ms=>setTimeout(()=>{wireLibraryNav();replaceGetAppWithStatus()},ms));
})();